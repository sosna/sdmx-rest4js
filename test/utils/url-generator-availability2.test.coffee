...
describe 'URL Generator for SDMX 3.0 availability queries', ->
  ...
  it 'generates a URL for a full availability query', ->
    expected = 'http://test.com/availability/dataflow/*/EXR/*/A..EUR.SP00.A/FREQ?\
  ...
  it 'generates a URL for a partial availability query', ->
    expected = 'http://test.com/availability/dataflow/*/EXR/*/*/*?\
  ...
  it 'generates a URL with all default values', ->
    expected = 'http://test.com/availability/*/*/*/*/*/*?\
    mode=exact&references=none'
  ...
  it 'offers to skip default values (context)', ->
    expected = "http://test.com/availability/dataflow"
  ...
  it 'offers to skip default values (agency)', ->
    expected = "http://test.com/availability/*/BIS"
  ...
  it 'offers to skip default values (artefact)', ->
    expected = "http://test.com/availability/*/*/EXR"
  ...
  it 'offers to skip default values (key)', ->
    expected = 'http://test.com/availability/dataflow/*/EXR/*/A.CH'
  ...
  it 'offers to skip default values (component)', ->
    expected = 'http://test.com/availability/dataflow/*/EXR/*/*/FREQ'
  ...
  it 'offers to skip default values (filters)', ->
    expected = "http://test.com/availability?c[REF_AREA]=UY,AR"
    query = AvailabilityQuery2.from({filters: 'REF_AREA=UY,AR'})
    service = Service.from({url: 'http://test.com', api: ApiVersion.v2_0_0})
    url = new UrlGenerator().getUrl(query, service, true)
    url.should.equal expected
